...
export const LazyDataTable :  React.FC<Props> = (props) => {
    ...
    const loadData = () => {
        props.fetchData(currRecord, rows, filters).then((response) => {
                const newData = parseDates(response.data);
                setItems(newData);
                setTotalRecords(response.data.length);
                setShowTable(response.data.length > 0 || props.columnOrder !== undefined);
                setLoading(false);
        })
